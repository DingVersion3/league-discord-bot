...
async def save_match(discord_id: int, puuid: str, match_id: str, champion: str, win: bool,
                      ...
                      position: str = "", enemy_champion: str | None = None, team_id: int = 0,
                      team_damage: int = 0, vision_score: int = 0) -> bool:
    async with _connect() as db:
        cursor = await db.execute(
            """
            INSERT INTO matches
                (match_id, discord_id, champion, win, kills, deaths, assists, damage, played_at, duration, cs, gold,
                doubleKills, tripleKills, quadraKills, pentaKills, position, enemy_champion, team_id, team_damage, vision_score)
            SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
            FROM users
            WHERE discord_id = ? AND puuid = ?
