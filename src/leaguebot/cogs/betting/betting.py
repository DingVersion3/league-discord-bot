...
async def place_bet(bet_id: int, bettor_discord_id: int, tracked_discord_id: int,
    ...
    return await db_place_wager(bet_id, bettor_discord_id, prediction, amount)


async def resolve(bet_id: int, tracked_won: bool) -> list[dict]:
    ...
    for wager in wagers:
        ...
        if correct:
            await adjust_wallet(wager["bettor_discord_id"], payout)
